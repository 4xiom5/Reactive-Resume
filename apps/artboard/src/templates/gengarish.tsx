...
const Header = () => {
  ...
  return (
    <div className="p-custom space-y-4 bg-primary text-background">
      <Picture className="border-background mx-auto" />
...
const Section = <T,>({
  ...
  return (
    <section id={section.id} className="grid">
      <h4 className="mb-2 border-b border-primary text-xl font-bold">{section.name}</h4>
                ...
                {keywords && keywords.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {keywords.map((keyword, index) => (
                      <span
                        key={index}
                        className="whitespace-nowrap rounded bg-white px-2 py-1 text-sm"
                      >
                        {keyword}
                      </span>
                    ))}
      ...
      {(item) => (
        <div>
          {isUrl(item.url.href) ? (
            <Link
              url={item.url}
              label={item.url.href.replace(/^https:\/\//, "")}
...
export const Gengarish = ({ columns, isFirstPage = false }: TemplateProps) => {
